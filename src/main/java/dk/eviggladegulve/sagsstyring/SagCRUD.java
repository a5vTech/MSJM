...
public class SagCRUD {
    ...
    /**
     * This method inserts a case to the database ans takes a case as parameter
    ...
    /**
     * This method inserts an adress to the database and takes 4 parameters.
    ...
    public ArrayList<Sag> getAllActiveCases() {
        ...
        String selectSQL = "SELECT * FROM eggNormaliseret.sag JOIN adresse ON (sag.adresse = adresse.adresse_id) JOIN byer ON (adresse.by_id = byer.by_id) WHERE sag.status = 1";
        try {
            PreparedStatement preparedStatement = con.prepareStatement(selectSQL);
            ResultSet rs = preparedStatement.executeQuery();
            if (rs != null) {
    ...
    /**
     * This method finds the last generatedased
     * @return INT
     */
    ...
    /**
     * This method finds a case by id andit can either be active or
     * @param id INT
    ...
    /**
     * This method shows all registered hours for an employeeon a specific case
     * @param sags_id INT
     * @param medarbejder_id INT
    ...
    public ArrayList<Sag> getAllEndedCases() {
        ...
        String selectSQL = "SELECT * FROM sag JOIN adresse ON (sag.adresse = adresse.adresse_id) JOIN byer ON (adresse.by_id = byer.by_id) WHERE sag.status = 0";
        try {
            PreparedStatement preparedStatement = con.prepareStatement(selectSQL);
            ResultSet rs = preparedStatement.executeQuery();
            if (rs != null) {
